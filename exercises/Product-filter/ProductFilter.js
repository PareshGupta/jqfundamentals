function JSONData() {
  ...
  this.createGrid = function() {
    ...
    for(var i = 0; i < this.cacheProductData.length; i++) {
      $("<img>", { src : "images/" + this.cacheProductData[i]["url"]}).attr("data-brands", this.setClassAttribute(this.cacheProductData[i]["brand"]))
                                                                      .attr("data-colors", this.cacheProductData[i]["color"].toLowerCase())
                                                                      .attr("data-available", this.cacheProductData[i]["sold_out"])
                                                                      .appendTo($grid);
    }
    $grid.appendTo("body");
  ...
  // displaying products based on filters
  this.displayProductsBasedOnFilters = function() {
    ...
    // pushing the checked brand values to array
    $("#Brands input:checked").each(function() {
      brandArray.push("[data-brands = '" + $(this).val() + "']");
    });
    selectorArray = this.addToFilters(selectorArray, brandArray);

    // pushing the checked color values to array
    $("#Colors input:checked").each(function() {
      colorArray.push("[data-colors = '" + $(this).val() + "']");
    });
    selectorArray = this.addToFilters(selectorArray, colorArray);

    // pushing the checked available value to array
    if($("#available:checked").length) {
      availableArray.push("[data-available = '" + $("#available").val() + "']");
    }
  ...
  // method to create selector string
  ...
  // method to create the array based on checked boxes being checked
